...
int main(int argc, char **argv) {
	...
	workload = std::unique_ptr<OV::SSDMobileNet> (new OV::SSDMobileNet());
        ...
        image_width = 300;
        num_channels = 3;
    } else if (FLAGS_model_name.compare("ssd-mobiledet") == 0) {
        post_processor = std::bind(&Processors::postProcessSSDMobiledet, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5, std::placeholders::_6);

        workload = std::unique_ptr<OV::SSDMobileDet>(new OV::SSDMobileDet());
        image_format = "NCHW";
        image_height = 320;
        image_width = 320;
        num_channels = 3;
