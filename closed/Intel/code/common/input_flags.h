...
static bool validate_user_config(const char* user_conf, const std::string& user_conf_value){
    ...
    if (user_conf_value.length() == 0){
        std::cout << "[WARNING]: --user_conf not provided. Will use mlperf default config (if --mlperf_conf provided)" << std::endl;
        return true;
    }

    boost::filesystem::path p( user_conf_value );
    if (!boost::filesystem::exists( p ) ){
        std::cout << "[WARNING]: Provided user config '" << user_conf_value << "' could not be found. Will use mlperf default config (if --mlperf_conf provided)" << std::endl;
    }
...
static const char logs_dir_message[] = "MLPerf output logs directory";

static bool validate_logs_dir(const char* logs_dir, const std::string& logs_dir_value) {

    if (logs_dir_value.length() == 0) return false;

    boost::filesystem::path p(logs_dir_value);
    if (!(boost::filesystem::exists(p))) {
...
DEFINE_string(logs_dir, "logs", logs_dir_message);
//DEFINE_validator(logs_dir, &validate_logs_dir);
...
static bool validate_model_name( const char*, const std::string& model_name_value){
    ...
    std::list<std::string> model_names = {"resnet50", "mobilenet", "ssd-mobilenet", "ssd-resnet34", "deeplabv3", "bert", "mobilebert", "mobilenet-edge", "ssd-mobilenet-v2", "ssd-mobiledet"};
