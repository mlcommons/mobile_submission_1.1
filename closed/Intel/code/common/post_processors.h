...
void postProcessSSDMobiledet(Item qitem, InferenceEngine::InferRequest req, std::vector<float> &results, std::vector<mlperf::ResponseId> &response_ids, unsigned batch_size,  std::vector<unsigned> &counts) {
    ...
    size_t j = 0;
    int object_size_ = 7;
    int max_proposal_count_ = out->size() / object_size_; //10 * batch_size;
